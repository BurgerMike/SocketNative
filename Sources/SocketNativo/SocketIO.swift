...
public final class ChatClient: NSObject {
    ...
    private var offlineQ: [(String, Any?, ((Any?) -> Void)?)] = []
    ...
    public func of(_ name: String) -> Namespace {
        ...
        let ns = Namespace(name: name); namespaces[name] = ns; return ns
    }
    ...
    public func emit(_ event: String, _ payload: Any?) { emit(event, payload, ack: nil) }
    public func emit<T: Encodable>(_ event: String, json: T, ack: ((Any?) -> Void)? = nil) { emit(event, AnyEncodable(json), ack: ack) }

    public func emit(_ event: String, _ payload: Any?, ack: ((Any?) -> Void)?) {
        ...
        guard readyForEmits else {
            if cfg.offlineQueue.enabled { if offlineQ.count >= cfg.offlineQueue.maxItems { _ = offlineQ.removeFirst() }; offlineQ.append((ev, pl, ack)) }
            return
        }
    ...
    private func flushOffline() { guard !offlineQ.isEmpty else { return }; let items = offlineQ; offlineQ.removeAll(); for (ev,pl,ack) in items { emit(ev, pl, ack: ack) } }
    ...
    private func handleText(_ text: String, fromProbe: Bool = false) {
        ...
        case .open:
            if let data = rest.data(using: .utf8), let open = try? JSONDecoder().decode(OpenPayload.self, from: data) {
                ...
                var s = "40"; let nsp = cfg.namespace; if nsp != "/" { s += nsp + "," }; driver?.sendText(s)
    ...
    private func handleBinary(_ data: Data, fromProbe: Bool = false) {
        if let (ev, args) = binAssembler.appendBinary(data) {
            onAny?(ev, args.first); of(cfg.namespace).dispatch(event: ev, payload: args.first, ack: nil)
        }
    }
    ...
    private func handleSocketIO(_ s: String) {
        ...
        case .connect:
            ...
            for (ev, pl) in stickyJoins { emit(ev, pl) }; flushOffline()
        ...
        case .event:
            if let data = jsonPart.data(using: .utf8), let arr = try? JSONSerialization.jsonObject(with: data) as? [Any],
               arr.count >= 1, let evName = arr[0] as? String {
                var payload = arr.count >= 2 ? arr[1] : nil
        ...
        case .binaryEvent, .binaryAck:
            ...
            _ = binAssembler.startPacket(isAck: (siotype == .binaryAck), nsp: nsp2, id: idLocal, header: countStr, jsonArray: arr)
